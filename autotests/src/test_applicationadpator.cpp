...
void ApplicationAdaptorTest::SetUp()
{
    g_ut_adpator_canReadable = true;
    g_ut_adpator_isImage = true;
}

void ApplicationAdaptorTest::TearDown() {}

// 构造函数: 传入有效 FileControl, 内部 fileControl 应被保存
TEST_F(ApplicationAdaptorTest, Construct_WithFileControl)
{
    FileControl control;
...
TEST_F(ApplicationAdaptorTest, Construct_WithNullptr_FileControlIsNull)
{
...
TEST_F(ApplicationAdaptorTest, OpenImageFile_NullFileControl_ReturnsFalse)
{
    ApplicationAdaptor adaptor(nullptr);
...
TEST_F(ApplicationAdaptorTest, OpenImageFile_NotReadable_ReturnsFalse)
{
    FileControl control;
    ApplicationAdaptor adaptor(&control);
...
TEST_F(ApplicationAdaptorTest, OpenImageFile_ReadableButNotImage_ReturnsFalse)
{
    FileControl control;
    ApplicationAdaptor adaptor(&control);
    QSignalSpy spy(&control, &FileControl::openImageFile);
...
TEST_F(ApplicationAdaptorTest, OpenImageFile_ReadableAndImage_PlainPath_ReturnsTrue)
{
    FileControl control;
    ApplicationAdaptor adaptor(&control);
    QSignalSpy spy(&control, &FileControl::openImageFile);
...
TEST_F(ApplicationAdaptorTest, OpenImageFile_ReadableAndImage_FileUrl_ReturnsTrue)
{
    FileControl control;
    ApplicationAdaptor adaptor(&control);
    QSignalSpy spy(&control, &FileControl::openImageFile);
