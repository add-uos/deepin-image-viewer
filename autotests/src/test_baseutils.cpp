...
// ---------------- hash ----------------
TEST_F(BaseUtilsTest, Hash_WhenSameInput_ReturnsSameMd5Hex)
{
...
TEST_F(BaseUtilsTest, Hash_WhenDifferentInput_ReturnsDifferentMd5)
{
    EXPECT_NE(hash("hello"), hash("world"));
}

TEST_F(BaseUtilsTest, Hash_WhenEmptyString_ReturnsMd5OfEmpty)
{
    // MD5 of empty string
...
TEST_F(BaseUtilsTest, StringWidth_WhenNonEmptyString_ReturnsPositiveValue)
{
    QFont font;
...
TEST_F(BaseUtilsTest, StringWidth_WhenEmptyString_ReturnsNonNegative)
{
    QFont font;
...
TEST_F(BaseUtilsTest, StringHeight_WhenNonEmptyString_ReturnsPositiveValue)
{
    QFont font;
...
// ---------------- timeToString ----------------
TEST_F(BaseUtilsTest, TimeToString_WhenNormalFormat_ReturnsNormalFormattedString)
{
...
TEST_F(BaseUtilsTest, TimeToString_WhenExifFormat_ReturnsExifFormattedString)
{
...
// ---------------- stringToDateTime ----------------
TEST_F(BaseUtilsTest, StringToDateTime_WhenExifFormat_ReturnsValidDateTime)
{
...
TEST_F(BaseUtilsTest, StringToDateTime_WhenNormalFormat_ReturnsValidDateTime)
{
...
TEST_F(BaseUtilsTest, StringToDateTime_WhenInvalidString_ReturnsInvalidDateTime)
{
...
// ---------------- getFileContent ----------------
TEST_F(BaseUtilsTest, GetFileContent_WhenFileReadable_ReturnsContent)
{
    QTemporaryFile tmp;
    tmp.open();
...
TEST_F(BaseUtilsTest, GetFileContent_WhenFileNotReadable_ReturnsEmpty)
{
    EXPECT_EQ(getFileContent("/nonexistent/path/file.txt"), QString(""));
}

// ---------------- SpliteText ----------------
TEST_F(BaseUtilsTest, SpliteText_WhenTextFitsLabel_ReturnsOriginalText)
{
    QFont font;
...
TEST_F(BaseUtilsTest, SpliteText_WhenTextExceedsLabel_ReturnsSplitTextWithNewline)
{
    QFont font;
...
TEST_F(BaseUtilsTest, SpliteText_WhenBReturnTrue_ReplacesSpacesWithNewlines)
{
    QFont font;
    const int narrow = 20;
...
// ---------------- onMountDevice ----------------
TEST_F(BaseUtilsTest, OnMountDevice_WhenMediaPath_ReturnsTrue)
{
    EXPECT_TRUE(onMountDevice("/media/user/usb/file.png"));
}

TEST_F(BaseUtilsTest, OnMountDevice_WhenRunMediaPath_ReturnsTrue)
{
    EXPECT_TRUE(onMountDevice("/run/media/user/usb/file.png"));
}

TEST_F(BaseUtilsTest, OnMountDevice_WhenHomePath_ReturnsFalse)
{
    EXPECT_FALSE(onMountDevice("/home/user/file.png"));
}

// ---------------- mountDeviceExist ----------------
TEST_F(BaseUtilsTest, MountDeviceExist_WhenPathUnderExistingDir_ReturnsTrue)
{
...
TEST_F(BaseUtilsTest, MountDeviceExist_WhenNonMediaPath_ReturnsExistenceOfEmptyMountPoint)
{
...
// ---------------- renderSVG ----------------
TEST_F(BaseUtilsTest, RenderSVG_WhenFileNotReadable_ReturnsNullPixmap)
{
    const QPixmap pix = renderSVG("/nonexistent/file.svg", QSize(32, 32));
    EXPECT_TRUE(pix.isNull());
}

TEST_F(BaseUtilsTest, RenderSVG_WhenValidSvg_ReturnsNonNullPixmap)
{
...
// ---------------- showInFileManager ----------------
TEST_F(BaseUtilsTest, ShowInFileManager_WhenPathEmpty_ReturnsImmediately)
{
    // 覆盖空路径早返回分支,无副作用
    showInFileManager("");
    SUCCEED();
...
TEST_F(BaseUtilsTest, ShowInFileManager_WhenPathNotExists_ReturnsImmediately)
{
...
TEST_F(BaseUtilsTest, CopyImageToClipboard_WhenSinglePath_SetsClipboardWithoutCrash)
{
...
TEST_F(BaseUtilsTest, CopyImageToClipboard_WhenMultiplePaths_SetsClipboardWithoutCrash)
{
    QStringList paths;
    paths << "/tmp/a.png"
          << ""
...
// ---------------- trashFile ----------------
TEST_F(BaseUtilsTest, TrashFile_WhenFileNotExists_ReturnsFalse)
{
    EXPECT_FALSE(trashFile("/nonexistent/file_to_trash.png"));
}

TEST_F(BaseUtilsTest, TrashFile_WhenFileExists_MovesToTrashAndReturnsTrue)
{
