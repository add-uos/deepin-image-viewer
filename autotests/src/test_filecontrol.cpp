...
void FileControlTest::SetUp()
{
}

void FileControlTest::TearDown()
{
}

// 测试构造与析构
TEST_F(FileControlTest, Construct)
{
...
// 测试获取标准图片路径
TEST_F(FileControlTest, StandardPicturesPath)
{
    FileControl control;
...
// 测试获取文件名（不要求文件存在）
TEST_F(FileControlTest, SlotGetFileName)
{
    FileControl control;
...
TEST_F(FileControlTest, SlotFileSuffix)
{
    FileControl control;
...
// 测试判断是否为图片
TEST_F(FileControlTest, IsImage)
{
    FileControl control;
...
// 测试配置读写
TEST_F(FileControlTest, ConfigValue)
{
    FileControl control;
...
TEST_F(FileControlTest, ResetImageFiles_SetAndClear_NoCrash)
{
    QTemporaryDir dir;
...
TEST_F(FileControlTest, AddImageFile_AppendWatch_NoCrash)
{
    QTemporaryDir dir;
    ASSERT_TRUE(dir.isValid());
...
TEST_F(FileControlTest, GetDirImagePath_WithImages_ReturnsList)
{
    QTemporaryDir dir;
    ASSERT_TRUE(dir.isValid());
...
TEST_F(FileControlTest, GetDirImagePath_EmptyPath_ReturnsEmpty)
{
    FileControl control;
...
TEST_F(FileControlTest, IsCurrentWatcherDir_UnknownDir_ReturnsFalse)
{
    FileControl control;
...
TEST_F(FileControlTest, SlotGetInfo_RealImage_ReturnsNonEmpty)
{
    QTemporaryDir dir;
    ASSERT_TRUE(dir.isValid());
...
TEST_F(FileControlTest, SlotGetFileNameSuffix_ReturnsFullName)
{
    QTemporaryDir dir;
    ASSERT_TRUE(dir.isValid());
...
TEST_F(FileControlTest, GetNamePath_ConstructsNewPath)
{
    FileControl control;
...
TEST_F(FileControlTest, DisplayinFileManager_DBusAvailable_ReturnsTrue)
{
    FileControl control;
...
// copyImage: 复制图片到剪贴板
TEST_F(FileControlTest, CopyImage_SetsClipboard_NoCrash)
{
    FileControl control;
...
// copyText: 复制文本到剪贴板
TEST_F(FileControlTest, CopyText_SetsClipboard)
{
    FileControl control;
...
TEST_F(FileControlTest, OcrImage_StaticImage_NoCrash)
{
    QTemporaryDir dir;
    ASSERT_TRUE(dir.isValid());
...
TEST_F(FileControlTest, ShowPrintDialog_Stubbed_NoCrash)
{
    FileControl control;
    Stub stub;
...
TEST_F(FileControlTest, ParseCommandlineGetPath_NoImageInArgs_ReturnsEmpty)
{
    FileControl control;
...
TEST_F(FileControlTest, IsCheckOnly_FirstCall_ReturnsTrue)
{
    FileControl control;
...
TEST_F(FileControlTest, IsSupportSetWallpaper_PngReadable_True_Other_False)
{
    QTemporaryDir dir;
    ASSERT_TRUE(dir.isValid());
...
TEST_F(FileControlTest, IsCanSupportOcr_StaticImage_ReturnsTrue)
{
    QTemporaryDir dir;
    ASSERT_TRUE(dir.isValid());
...
TEST_F(FileControlTest, IsCanReadable_ExistingFile_True_Missing_False)
{
    QTemporaryDir dir;
    ASSERT_TRUE(dir.isValid());
...
TEST_F(FileControlTest, IsRotatable_MissingFile_False_RealFile_Callable)
{
    QTemporaryDir dir;
    ASSERT_TRUE(dir.isValid());
...
TEST_F(FileControlTest, IsCanWrite_LocalFile_Callable)
{
    QTemporaryDir dir;
    ASSERT_TRUE(dir.isValid());
...
TEST_F(FileControlTest, IsCanDelete_LocalFile_Callable)
{
    QTemporaryDir dir;
    ASSERT_TRUE(dir.isValid());
...
TEST_F(FileControlTest, SlotFileReName_Success_EmitsSignal)
{
    QTemporaryDir dir;
    ASSERT_TRUE(dir.isValid());
...
TEST_F(FileControlTest, SlotFileReName_Nonexistent_ReturnsFalse)
{
    FileControl control;
...
TEST_F(FileControlTest, IsShowToolTip_SameName_ReturnsFalse)
{
    QTemporaryDir dir;
    ASSERT_TRUE(dir.isValid());
...
// saveSetting: 直接调用不崩溃
TEST_F(FileControlTest, SaveSetting_DirectCall_NoCrash)
{
    FileControl control;
    control.saveSetting();
    SUCCEED();
...
TEST_F(FileControlTest, GetLastWidthHeight_NotLessThanMinimum)
{
    FileControl control;
...
TEST_F(FileControlTest, SetSettingWidthHeight_NoCrash)
{
    FileControl control;
...
TEST_F(FileControlTest, GetPrimaryScreenCenter_ReturnsValue)
{
    FileControl control;
...
TEST_F(FileControlTest, EnableNavigation_ReadWriteRoundTrip)
{
    FileControl control;
...
TEST_F(FileControlTest, GetCompanyLogo_ReturnsValidUrl)
{
    FileControl control;
...
TEST_F(FileControlTest, ShowShortcutPanel_NoCrash)
{
    FileControl control;
...
TEST_F(FileControlTest, TerminateShortcutPanelProcess_Idle_NoCrash)
{
    FileControl control;
    control.terminateShortcutPanelProcess();
    SUCCEED();
...
TEST_F(FileControlTest, CreateShortcutString_BuildsAndCaches)
{
    FileControl control;
...
TEST_F(FileControlTest, SetWallpaper_NullPath_NoCrash)
{
    FileControl control;
