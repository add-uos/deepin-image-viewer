...
void FileTrashHelperTest::SetUp() {}
void FileTrashHelperTest::TearDown() {}

// ==================== 构造函数 ====================

// 测试构造函数初始化 DBus 设备管理接口
TEST_F(FileTrashHelperTest, Constructor_InitializesDeviceManager)
{
...
// 测试删除不存在的文件返回 false
TEST_F(FileTrashHelperTest, RemoveFile_NonExistentFile_ReturnsFalse)
{
    FileTrashHelper helper;
...
TEST_F(FileTrashHelperTest, RemoveFile_ExistingFile_ReturnsTrue)
{
...
TEST_F(FileTrashHelperTest, ResetMountInfo_ClearsState)
{
    FileTrashHelper helper;
...
// 测试无效 URL 返回 false
TEST_F(FileTrashHelperTest, IsGvfsFile_InvalidUrl_ReturnsFalse)
{
    FileTrashHelper helper;
    QUrl invalidUrl;
...
// 测试普通本地路径返回 false
TEST_F(FileTrashHelperTest, IsGvfsFile_NormalPath_ReturnsFalse)
{
    FileTrashHelper helper;
...
TEST_F(FileTrashHelperTest, IsGvfsFile_RunUserGvfsPath_ReturnsTrue)
{
    FileTrashHelper helper;
...
TEST_F(FileTrashHelperTest, IsGvfsFile_RootGvfsPath_ReturnsTrue)
{
    FileTrashHelper helper;
...
TEST_F(FileTrashHelperTest, IsGvfsFile_SmbMountsPath_ReturnsTrue)
{
    FileTrashHelper helper;
...
// 测试无挂载设备时返回 false
TEST_F(FileTrashHelperTest, IsExternalDevice_NoMounts_ReturnsFalse)
{
    FileTrashHelper helper;
...
// 测试路径匹配外部设备时返回 true
TEST_F(FileTrashHelperTest, IsExternalDevice_PathOnExternal_ReturnsTrue)
{
    FileTrashHelper helper;
    helper.mountDevices.clear();
...
TEST_F(FileTrashHelperTest, IsExternalDevice_PathNotOnExternal_ReturnsFalse)
{
    FileTrashHelper helper;
    helper.mountDevices.clear();
    helper.mountDevices.insert("dev1", "/mnt/usb");
...
TEST_F(FileTrashHelperTest, QueryMountInfo_AlreadyInitialized_NoDBusCall)
{
    FileTrashHelper helper;
    helper.initData = true;
...
TEST_F(FileTrashHelperTest, QueryMountInfo_DBusFails_NoCrash)
{
    FileTrashHelper helper;
...
TEST_F(FileTrashHelperTest, FileCanTrash_GvfsPath_ReturnsFalse)
{
    FileTrashHelper helper;
    QUrl url = QUrl::fromLocalFile("/run/user/1000/gvfs/smb:test/file.png");
...
// 测试普通本地路径可回收
TEST_F(FileTrashHelperTest, FileCanTrash_NormalLocalPath_ReturnsTrue)
{
    FileTrashHelper helper;
    QTemporaryFile tmpFile;
    ASSERT_TRUE(tmpFile.open());
...
// 测试同一目录连续调用不重复重置挂载信息
TEST_F(FileTrashHelperTest, FileCanTrash_SameDirNotResetMountInfo)
{
    FileTrashHelper helper;
    QTemporaryFile tmpFile;
    ASSERT_TRUE(tmpFile.open());
...
TEST_F(FileTrashHelperTest, MoveFileToTrashWithDBus_InvalidUrl_ReturnsInvalidUrlError)
{
    FileTrashHelper helper;
    QUrl invalidUrl;
...
TEST_F(FileTrashHelperTest, MoveFileToTrash_DBusSuccess_ReturnsTrue)
{
    FileTrashHelper helper;
    Stub stub;
    ...
    QUrl url = QUrl::fromLocalFile("/tmp/FileTrashHelperTest_test.png");
    EXPECT_TRUE(helper.moveFileToTrash(url));
}

// 测试 DBus 接口失败且回退接口(trashFile)也失败时返回 false
TEST_F(FileTrashHelperTest, MoveFileToTrash_DBusErrorAndFallbackFails_ReturnsFalse)
{
    FileTrashHelper helper;
    Stub stub;
    ...
    QUrl url = QUrl::fromLocalFile("/tmp/FileTrashHelperTest_nonexistent_12345.png");
...
TEST_F(FileTrashHelperTest, MoveFileToTrash_DBusErrorAndFallbackSucceeds_ReturnsTrue)
{
    FileTrashHelper helper;
    Stub stub;
    g_ut_stubMoveDBusRet = kUtTrashInterfaceError;
    ...
    QUrl url = QUrl::fromLocalFile("/tmp/FileTrashHelperTest_fallback_test.png");
    EXPECT_TRUE(helper.moveFileToTrash(url));
}

// 测试 DBus 返回 kTrashInvalidUrl（非 InterfaceError 非 Success）时返回 false
TEST_F(FileTrashHelperTest, MoveFileToTrash_DBusInvalidUrl_ReturnsFalse)
{
    FileTrashHelper helper;
    Stub stub;
    ...
    QUrl url = QUrl::fromLocalFile("/tmp/FileTrashHelperTest_invalidurl_test.png");
    EXPECT_FALSE(helper.moveFileToTrash(url));
}
