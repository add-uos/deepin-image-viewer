...
void ImageFileWatcherTest::SetUp()
{
    // 每个用例前重置单例状态，保证隔离
    ImageFileWatcher::instance()->resetImageFiles({});
}

void ImageFileWatcherTest::TearDown()
{
    ImageFileWatcher::instance()->resetImageFiles({});
}

// QFileSystemWatcher 依赖 inotify（Linux）。在并发代理/容器等环境中
// inotify 实例或 watch 数可能耗尽，导致 addPath 静默失败。提供探针供依赖
// “目录已被监控”断言的用例判断是否应跳过。
static bool watcherCanAddPaths()
{
    QFileSystemWatcher probe;
    QString dir = QDir::tempPath() + "/ut_watchprobe_" +
                  QString::number(QCoreApplication::applicationPid());
    QDir().mkpath(dir);
    QString f = dir + "/probe.png";
    QImage img(2, 2, QImage::Format_ARGB32);
    img.save(f, "PNG");
    bool ok = probe.addPath(f);
    return ok;
}

// 测试 instance() 返回同一单例
TEST_F(ImageFileWatcherTest, InstanceReturnsSameSingleton)
{
...
TEST_F(ImageFileWatcherTest, ResetImageFilesEmpty)
{
    ImageFileWatcher::instance()->resetImageFiles({});
    SUCCEED();
}
...
TEST_F(ImageFileWatcherTest, ResetImageFilesWithExistingFiles)
{
...
TEST_F(ImageFileWatcherTest, ResetImageFilesWithNonexisting)
{
...
// 测试 isCurrentDir
TEST_F(ImageFileWatcherTest, IsCurrentDir)
{
    if (!watcherCanAddPaths())
        GTEST_SKIP() << "QFileSystemWatcher unavailable (inotify limit exhausted in this environment)";
...
TEST_F(ImageFileWatcherTest, FileRenameHitAndMiss)
{
...
TEST_F(ImageFileWatcherTest, RecordAndClearRotateStatus)
{
...
TEST_F(ImageFileWatcherTest, ImageFileChangedSignal)
{
...
TEST_F(ImageFileWatcherTest, PrivateOnImageFileChangedCached)
{
...
TEST_F(ImageFileWatcherTest, PrivateOnImageFileChangedNotCached)
{
...
TEST_F(ImageFileWatcherTest, PrivateOnImageDirChanged)
{
    QString dir = QDir::tempPath() + "/ut_filewatcher_" +
                  QString::number(QCoreApplication::applicationPid());
