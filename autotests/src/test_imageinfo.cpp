...
void ImageInfoTest::SetUp()
{
}

void ImageInfoTest::TearDown()
{
}

// 测试默认构造
TEST_F(ImageInfoTest, DefaultConstruct)
{
    ImageInfo info;
...
// 测试设置 source
TEST_F(ImageInfoTest, SetSource)
{
    ImageInfo info;
...
// 测试设置 frameIndex
TEST_F(ImageInfoTest, SetFrameIndex)
{
    ImageInfo info;
...
// 测试运行时属性默认值
TEST_F(ImageInfoTest, RuntimeProperties)
{
    ImageInfo info;
...
// 测试带 source 的构造函数
TEST_F(ImageInfoTest, ConstructWithSource)
{
    QString path = makeTempPng("ImageInfoTest_ctor.png");
...
TEST_F(ImageInfoTest, TypeNoData)
{
    ImageInfo info;
...
// 测试加载真实图片后的各项信息
TEST_F(ImageInfoTest, LoadRealImageInfo)
{
    QString path = makeTempPng("ImageInfoTest_load.png");
    ImageInfo info;
...
TEST_F(ImageInfoTest, SwapWidthAndHeight)
{
    // 无数据：不崩溃，无效果
    ImageInfo empty;
    ...
    // 有数据：使用非正方形图片以便观察交换
    QString path = makeTempPng("ImageInfoTest_swap.png", 12, 8);
    ImageInfo info;
    info.setSource(QUrl::fromLocalFile(path));
    EXPECT_TRUE(waitUntilStatus(info, ImageInfo::Ready));
    if (info.data) {
...
// 测试 scale 运行时属性
TEST_F(ImageInfoTest, ScaleProperty)
{
    ...
    // 有数据：可读写
    QString path = makeTempPng("ImageInfoTest_scale.png");
    ImageInfo info;
    info.setSource(QUrl::fromLocalFile(path));
    EXPECT_TRUE(waitUntilStatus(info, ImageInfo::Ready));
    info.setScale(1.5);
...
// 测试 setX / setY 运行时属性
TEST_F(ImageInfoTest, XYProperties)
{
    ...
    // 有数据：可读写
    QString path = makeTempPng("ImageInfoTest_xy.png");
    ImageInfo info;
    info.setSource(QUrl::fromLocalFile(path));
    EXPECT_TRUE(waitUntilStatus(info, ImageInfo::Ready));
    info.setX(7.0);
...
// 测试 exists()
TEST_F(ImageInfoTest, Exists)
{
    ImageInfo empty;
    EXPECT_FALSE(empty.exists());

    QString path = makeTempPng("ImageInfoTest_exists.png");
    ImageInfo info;
    info.setSource(QUrl::fromLocalFile(path));
    EXPECT_TRUE(waitUntilStatus(info, ImageInfo::Ready));
    if (info.data) {
...
TEST_F(ImageInfoTest, HasCachedThumbnail)
{
    // 空 URL 分支
    ImageInfo empty;
    ...
    ImageInfo nonexist;
    nonexist.setSource(QUrl::fromLocalFile("/tmp/ImageInfoTest_nonexist_thumb.png"));
    waitUntilStatus(nonexist, ImageInfo::Error);
    EXPECT_FALSE(nonexist.hasCachedThumbnail());

    // 加载真实图片后缩略图已缓存 -> default 分支返回 true
    QString path = makeTempPng("ImageInfoTest_thumb.png");
    ImageInfo loaded;
...
// 测试 reloadData() 重新加载
TEST_F(ImageInfoTest, ReloadData)
{
    QString path = makeTempPng("ImageInfoTest_reload.png");
    ImageInfo info;
    info.setSource(QUrl::fromLocalFile(path));
    EXPECT_TRUE(waitUntilStatus(info, ImageInfo::Ready));
...
TEST_F(ImageInfoTest, ClearCurrentCache)
{
    // 无数据：no-op，不崩溃
    ImageInfo empty;
    ...
    // 有数据：调用不应崩溃且不影响已加载信息
    QString path = makeTempPng("ImageInfoTest_clearcur.png");
    ImageInfo info;
    info.setSource(QUrl::fromLocalFile(path));
    EXPECT_TRUE(waitUntilStatus(info, ImageInfo::Ready));
    if (info.data) {
...
// 测试静态方法 clearCache()
TEST_F(ImageInfoTest, ClearCacheStatic)
{
    // 仅验证可调用且不崩溃
...
TEST_F(ImageInfoTest, SetStatus)
{
    ImageInfo info;
...
TEST_F(ImageInfoTest, UpdateDataSamePointer)
{
    ImageInfo info;
    ...
    QString path = makeTempPng("ImageInfoTest_updatedata.png");
    info.setSource(QUrl::fromLocalFile(path));
    EXPECT_TRUE(waitUntilStatus(info, ImageInfo::Ready));
...
TEST_F(ImageInfoTest, UpdateDataDetectsChange)
{
    // 先加载不存在的文件：data.exist=false, type=NullImage
    ImageInfo info;
    info.setSource(QUrl::fromLocalFile("/tmp/ImageInfoTest_nonexist_for_change.png"));
    EXPECT_TRUE(waitUntilStatus(info, ImageInfo::Error));
    // data 可能为空（加载未完成，无事件循环）

    // 切换到真实图片，触发 updateData 检测到差异
    QString path = makeTempPng("ImageInfoTest_changedetected.png");
...
TEST_F(ImageInfoTest, RefreshDataFromCache)
{
    // 分支1：空路径 -> 设置 Error
    ImageInfo infoEmpty;
    ...
    infoNoCache.imageUrl = QUrl::fromLocalFile("/tmp/ImageInfoTest_uncached_refresh.png");
    ...
    infoReload.imageUrl = QUrl::fromLocalFile("/tmp/ImageInfoTest_uncached_reload.png");
    infoReload.refreshDataFromCache(true);
    EXPECT_EQ(infoReload.status(), ImageInfo::Loading);

    // 分支2：数据已缓存，新对象取缓存数据
    QString path = makeTempPng("ImageInfoTest_refresh_cached.png");
    ImageInfo donor;
...
TEST_F(ImageInfoTest, OnLoadFinished)
{
    ImageInfo info;
    info.imageUrl = QUrl::fromLocalFile("/tmp/ImageInfoTest_match.png");

    // 不匹配 -> 不处理，状态保持
    info.onLoadFinished("/tmp/ImageInfoTest_OTHER.png", 0);
    EXPECT_EQ(info.status(), ImageInfo::Null);

    // 匹配但缓存无数据 -> refreshDataFromCache(false) -> Error
    info.onLoadFinished("/tmp/ImageInfoTest_match.png", 0);
    EXPECT_EQ(info.status(), ImageInfo::Error);
}

// 测试 onSizeChanged() 槽
TEST_F(ImageInfoTest, OnSizeChanged)
{
    // data 为空：即使路径匹配也不发信号
    ImageInfo info;
    info.imageUrl = QUrl::fromLocalFile("/tmp/ImageInfoTest_size.png");
    QSignalSpy wSpy(&info, &ImageInfo::widthChanged);
    QSignalSpy hSpy(&info, &ImageInfo::heightChanged);
    info.onSizeChanged("/tmp/ImageInfoTest_size.png", 0);  // 匹配但 data 为空
    EXPECT_EQ(wSpy.count(), 0);
    EXPECT_EQ(hSpy.count(), 0);
    // 路径不匹配 -> 不处理
    info.onSizeChanged("/tmp/ImageInfoTest_other.png", 0);
    EXPECT_EQ(wSpy.count(), 0);
    EXPECT_EQ(hSpy.count(), 0);

    // 有数据且匹配 -> 发送 widthChanged/heightChanged
    QString path = makeTempPng("ImageInfoTest_onsize.png");
    ImageInfo loaded;
    loaded.setSource(QUrl::fromLocalFile(path));
    EXPECT_TRUE(waitUntilStatus(loaded, ImageInfo::Ready));
...
TEST_F(ImageInfoTest, ImageInfoCacheDeletingDestructor)
{
...
TEST_F(ImageInfoTest, ImageInfoDataIsError_DamagedImage)
{
    ImageInfoData data;
...
TEST_F(ImageInfoTest, ImageInfoDataIsError_NormalImage)
{
    ImageInfoData data;
...
TEST_F(ImageInfoTest, ImageInfoCacheRemoveCache)
{
...
TEST_F(ImageInfoTest, ImageInfoCacheLoadFinished)
{
    ImageInfoCache cache;
