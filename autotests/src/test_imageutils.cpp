...
// ---------------- imageSupportRead ----------------
TEST_F(ImageUtilsTest, ImageSupportRead_WhenCommonExt_ReturnsTrue)
{
    EXPECT_TRUE(imageSupportRead("/tmp/foo.png"));
    EXPECT_TRUE(imageSupportRead("/tmp/foo.jpg"));
}

TEST_F(ImageUtilsTest, ImageSupportRead_WhenIcnsExt_ReturnsTrue)
{
    EXPECT_TRUE(imageSupportRead("/tmp/foo.icns"));
}

TEST_F(ImageUtilsTest, ImageSupportRead_WhenX3FExt_ReturnsFalse)
{
    EXPECT_FALSE(imageSupportRead("/tmp/foo.x3f"));
}

// ---------------- imageSupportSave ----------------
TEST_F(ImageUtilsTest, ImageSupportSave_DelegatesToUnionImageCanSave)
{
    QTemporaryDir dir;
...
TEST_F(ImageUtilsTest, ImageSupportSave_WhenUnsupportedExt_ReturnsFalse)
{
    EXPECT_FALSE(imageSupportSave("/tmp/foo.zzzunknown"));
}

// ---------------- rotate ----------------
TEST_F(ImageUtilsTest, Rotate_WhenAngleInvalid_ReturnsFalse)
{
    QTemporaryDir dir;
...
TEST_F(ImageUtilsTest, Rotate_WhenPngAndAngle90_ReturnsTrue)
{
    QTemporaryDir dir;
...
TEST_F(ImageUtilsTest, CutSquareImage_WhenSingleArg_DelegatesToTwoArgOverload)
{
    QPixmap src(10, 6);
    src.fill(Qt::blue);
...
TEST_F(ImageUtilsTest, CutSquareImage_WhenSizeProvided_ReturnsNonNullPixmap)
{
    QPixmap src(20, 20);
    src.fill(Qt::green);
...
// ---------------- getImagesInfo ----------------
TEST_F(ImageUtilsTest, GetImagesInfo_WhenNonRecursive_ReturnsFilesInDir)
{
    QTemporaryDir dir;
...
TEST_F(ImageUtilsTest, GetImagesInfo_WhenRecursive_ReturnsAllFiles)
{
    QTemporaryDir dir;
    makeTempPng(dir, "a.png");
...
// ---------------- getOrientation ----------------
TEST_F(ImageUtilsTest, GetOrientation_AlwaysReturnsOne)
{
    QTemporaryDir dir;
...
// ---------------- getRotatedImage ----------------
TEST_F(ImageUtilsTest, GetRotatedImage_WhenValidPng_ReturnsNonEmptyImage)
{
    QTemporaryDir dir;
...
TEST_F(ImageUtilsTest, GetRotatedImage_WhenFileNotExists_ReturnsNullImage)
{
...
// ---------------- scaleImage ----------------
TEST_F(ImageUtilsTest, ScaleImage_WhenValidImage_ReturnsScaledImage)
{
    QTemporaryDir dir;
...
TEST_F(ImageUtilsTest, ScaleImage_WhenUnsupportedFormat_ReturnsNullImage)
{
...
// ---------------- getCreateDateTime ----------------
TEST_F(ImageUtilsTest, GetCreateDateTime_WhenFileExists_ReturnsValidDateTime)
{
    QTemporaryDir dir;
...
// ---------------- getAllMetaData ----------------
TEST_F(ImageUtilsTest, GetAllMetaData_WhenValidPng_ReturnsPopulatedMap)
{
    QTemporaryDir dir;
...
// ---------------- cachePixmap ----------------
TEST_F(ImageUtilsTest, CachePixmap_WhenFileExists_ReturnsNonEmptyPixmap)
{
    QTemporaryDir dir;
...
TEST_F(ImageUtilsTest, CachePixmap_WhenCalledTwice_SecondFromCache)
{
    QTemporaryDir dir;
...
// ---------------- thumbnailCachePath ----------------
TEST_F(ImageUtilsTest, ThumbnailCachePath_ReturnsPathEndingWithThumbnails)
{
...
// ---------------- thumbnailPath ----------------
TEST_F(ImageUtilsTest, ThumbnailPath_WhenThumbNormal_ReturnsNormalSubdir)
{
...
TEST_F(ImageUtilsTest, ThumbnailPath_WhenThumbLarge_ReturnsLargeSubdir)
{
    const QString p = thumbnailPath("/tmp/foo.png", ThumbLarge);
    EXPECT_TRUE(p.contains("/large/"));
}

TEST_F(ImageUtilsTest, ThumbnailPath_WhenThumbFail_ReturnsFailSubdir)
{
...
// ---------------- thumbnailExist ----------------
TEST_F(ImageUtilsTest, ThumbnailExist_WhenNoCache_ReturnsFalse)
{
    // 用一个独特路径,确保缓存不存在
...
// ---------------- generateThumbnail ----------------
TEST_F(ImageUtilsTest, GenerateThumbnail_WhenValidPng_ReturnsTrueAndCreatesCache)
{
    QTemporaryDir dir;
...
// ---------------- getThumbnail ----------------
TEST_F(ImageUtilsTest, GetThumbnail_WhenCacheOnlyAndNoCache_ReturnsEmptyPixmap)
{
    const QPixmap pix = getThumbnail("/tmp/no_such_file_unique.png", true);
    EXPECT_TRUE(pix.isNull());
}

TEST_F(ImageUtilsTest, GetThumbnail_WhenCacheExists_ReturnsNonEmptyPixmap)
{
    QTemporaryDir dir;
...
// ---------------- removeThumbnail ----------------
TEST_F(ImageUtilsTest, RemoveThumbnail_WhenCalled_RemovesCacheFiles)
{
    QTemporaryDir dir;
...
TEST_F(ImageUtilsTest, SupportedImageFormats_ReturnsNonEmptyListWithGlobPrefix)
{
...
TEST_F(ImageUtilsTest, ImageSupportWallPaper_WhenSupportedPng_ReturnsTrue)
{
    QTemporaryDir dir;
...
TEST_F(ImageUtilsTest, ImageSupportWallPaper_WhenUnsupportedExt_ReturnsFalse)
{
    EXPECT_FALSE(imageSupportWallPaper("/tmp/file.zzzunknown"));
}

// ---------------- makeVaultLocalPath ----------------
TEST_F(ImageUtilsTest, MakeVaultLocalPath_WhenBaseProvided_UsesGivenBase)
{
...
TEST_F(ImageUtilsTest, MakeVaultLocalPath_WhenBaseEmpty_UsesDefaultBase)
{
    const QString p = makeVaultLocalPath("file.txt", "");
    EXPECT_TRUE(p.contains(VAULT_DECRYPT_DIR_NAME));
}

TEST_F(ImageUtilsTest, MakeVaultLocalPath_WhenPathStartsWithSlash_DoesNotDoubleSlash)
{
...
// ---------------- isVaultFile ----------------
TEST_F(ImageUtilsTest, IsVaultFile_WhenPathUnderVault_ReturnsTrue)
{
    const QString vaultPath = makeVaultLocalPath("file.txt", "");
    EXPECT_TRUE(isVaultFile(vaultPath));
}

TEST_F(ImageUtilsTest, IsVaultFile_WhenRegularPath_ReturnsFalse)
{
    EXPECT_FALSE(isVaultFile("/tmp/some_regular_file.png"));
}

// ---------------- isCanRemove ----------------
TEST_F(ImageUtilsTest, IsCanRemove_WhenRegularPath_ReturnsTrue)
{
    EXPECT_TRUE(isCanRemove("/tmp/some_regular_file.png"));
}

TEST_F(ImageUtilsTest, IsCanRemove_WhenVaultFile_ReturnsFalse)
{
    const QString vaultPath = makeVaultLocalPath("file.txt", "");
    EXPECT_FALSE(isCanRemove(vaultPath));
}

TEST_F(ImageUtilsTest, IsCanRemove_WhenTrashPath_ReturnsFalse)
{
