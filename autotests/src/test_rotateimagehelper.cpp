...
void RotateImageHelperTest::SetUp()
{
    // 重置单例旋转状态，保证用例间隔离
    RotateImageHelper::instance()->resetRotateState();
}

void RotateImageHelperTest::TearDown() {}

// ==================== instance() ====================

// 测试 instance() 返回非空单例
TEST_F(RotateImageHelperTest, Instance_ReturnsNonNull)
{
...
TEST_F(RotateImageHelperTest, Instance_ReturnsSameSingleton)
{
...
// 测试 0 度旋转被跳过（提前返回）
TEST_F(RotateImageHelperTest, RotateImageFile_ZeroAngle_ReturnsEarly)
{
...
// 测试 360 度等同于 0 度，被跳过
TEST_F(RotateImageHelperTest, RotateImageFile_FullRotation_ReturnsEarly)
{
...
TEST_F(RotateImageHelperTest, RotateImageFile_RealFile_EmitsFinishedSignal)
{
    // 准备临时图片文件
...
TEST_F(RotateImageHelperTest, ResetRotateState_OnSingleton_NoCrash)
{
    RotateImageHelper::instance()->resetRotateState();
    SUCCEED();
}
...
TEST_F(RotateImageHelperTest, ResetRotateState_NullData_NoCrash)
{
...
TEST_F(RotateImageHelperTest, ResetRotateState_WithDataInitialized_NoCrash)
{
    RotateImageHelper *fresh = new RotateImageHelper();
...
TEST_F(RotateImageHelperTest, RotateImageImpl_NonExistentSource_ReturnsFalse)
{
    QTemporaryDir tmpDir;
    ASSERT_TRUE(tmpDir.isValid());
...
// 测试对真实文件旋转成功
TEST_F(RotateImageHelperTest, RotateImageImpl_ValidFile_ReturnsTrue)
{
    QTemporaryDir tmpDir;
    ASSERT_TRUE(tmpDir.isValid());
...
TEST_F(RotateImageHelperTest, RotateImageImpl_CacheExists_SkipsCopy)
{
    QTemporaryDir tmpDir;
    ASSERT_TRUE(tmpDir.isValid());
...
TEST_F(RotateImageHelperTest, CheckDataValid_NewInstance_CreatesData)
{
    RotateImageHelper *fresh = new RotateImageHelper();
...
TEST_F(RotateImageHelperTest, CheckDataValid_AlreadyValid_NoDuplicateCreation)
{
    RotateImageHelper *fresh = new RotateImageHelper();
    fresh->checkDataValid();
...
TEST_F(RotateImageHelperTest, EnqueueRotateTask_QueuesAndProcesses)
{
    RotateImageHelper *fresh = new RotateImageHelper();
    fresh->checkDataValid();
