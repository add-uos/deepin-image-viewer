...
void ThumbnailCacheTest::SetUp()
{
}

void ThumbnailCacheTest::TearDown()
{
}

// 测试默认构造与析构
TEST_F(ThumbnailCacheTest, DefaultConstruct)
{
...
// 测试 instance() 返回同一单例
TEST_F(ThumbnailCacheTest, InstanceReturnsSameSingleton)
{
...
TEST_F(ThumbnailCacheTest, AddContainsAndGet)
{
    ThumbnailCache cache;
...
// 测试 get 未命中返回空图像
TEST_F(ThumbnailCacheTest, GetNonexistentReturnsNull)
{
    ThumbnailCache cache;
...
// 测试不同帧索引独立缓存
TEST_F(ThumbnailCacheTest, FrameIndexIndependence)
{
    ThumbnailCache cache;
...
// 测试 take 取出后从缓存移除
TEST_F(ThumbnailCacheTest, TakeRemovesEntry)
{
    ThumbnailCache cache;
...
// 测试 take 未命中返回空图像
TEST_F(ThumbnailCacheTest, TakeNonexistentReturnsNull)
{
    ThumbnailCache cache;
...
// 测试 remove
TEST_F(ThumbnailCacheTest, RemoveEntry)
{
    ThumbnailCache cache;
...
// 测试 setMaxCost 驱逐策略
TEST_F(ThumbnailCacheTest, SetMaxCostEvictsOldEntries)
{
    ThumbnailCache cache;
    cache.setMaxCost(1);
...
// 测试 clear 清空全部
TEST_F(ThumbnailCacheTest, ClearAll)
{
    ThumbnailCache cache;
...
// 测试 keys 返回所有键
TEST_F(ThumbnailCacheTest, KeysReturnsAllEntries)
{
    ThumbnailCache cache;
    QImage img(1, 1, QImage::Format_ARGB32);
    cache.add("k1", 0, img);
...
TEST_F(ThumbnailCacheTest, ToFindKeyCombinesPathAndFrame)
{
